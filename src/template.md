---
title: Commercial Rental Agreement
date: __AGREEMENT_DATE__
---

# Commercial Rental Agreement

**For Small Businesses and Startups**

This Agreement is made on **__AGREEMENT_DATE__** at **__CITY__**, India.

---

## Between

### LANDLORD (First Party)

**Name:** __LANDLORD_NAME__
**Address:** __LANDLORD_ADDRESS__
**Phone:** __LANDLORD_PHONE__

(Hereinafter referred to as "**Landlord**" or "**First Party**", which expression shall, unless repugnant to the context or meaning thereof, mean and include his/her heirs, executors, administrators, legal representatives, and assigns)

### AND

### TENANT (Second Party)

**Name:** __TENANT_NAME__
**Business Name:** __TENANT_BUSINESS_NAME__ *(if applicable)*
**Address:** __TENANT_ADDRESS__
**Phone:** __TENANT_PHONE__

(Hereinafter referred to as "**Tenant**" or "**Second Party**", which expression shall, unless repugnant to the context or meaning thereof, mean and include his/her heirs, executors, administrators, legal representatives, successors, and assigns)

---

## Recitals

**WHEREAS** the Landlord is the lawful owner and in possession of the property described below and is desirous of renting out the said property for commercial use;

**AND WHEREAS** the Tenant is desirous of taking the said property on rent for the purpose of conducting lawful business activities as described in this Agreement;

**NOW THEREFORE** in consideration of the mutual covenants, terms, and conditions set forth in this Agreement, both parties agree as follows:

---

## 1. Property Description

The Landlord agrees to rent out the following property to the Tenant:

**Property Address:**
__PROPERTY_ADDRESS__

**Floor:** __PROPERTY_FLOOR__ *(if applicable)*

**Area:** Approximately __PROPERTY_AREA_SQFT__ square feet *(if applicable)*

**Property Type:** Commercial office/retail space suitable for small business use

(Hereinafter referred to as "**the Property**")

---

## 2. Lease Period

**Start Date:** __LEASE_START_DATE__

**End Date:** __LEASE_END_DATE__

**Total Duration:** 11 months (to avoid mandatory registration requirements)

The tenancy shall commence on the Start Date and shall continue for a period of 11 months, ending on the End Date, unless terminated earlier in accordance with the terms of this Agreement.

> **Note:** This agreement is for 11 months to avoid the requirement of mandatory registration under the Registration Act, 1908. Agreements exceeding 11 months typically require registration with the Sub-Registrar office.

---

## 3. Rent and Payment Terms

**Monthly Rent:** ₹__MONTHLY_RENT__

**Payment Due Date:** __RENT_DUE_DATE__ of each month

**Payment Method:** Bank transfer, cheque, or cash (as mutually agreed)

**First Rent Payment:** The first month's rent shall be paid on or before the Start Date.

### Late Payment

If the Tenant fails to pay the rent by the due date, a grace period of 5 days shall be allowed. After the grace period, the Tenant shall pay a late payment penalty of 2% of the monthly rent for each week of delay.

### Rent Receipt

The Landlord shall provide a receipt for each rent payment made by the Tenant.

---

## 4. Security Deposit

**Security Deposit Amount:** ₹__SECURITY_DEPOSIT__ (typically 2-3 months' rent)

The Security Deposit shall be paid by the Tenant to the Landlord at the time of execution of this Agreement. This deposit serves as security for:
- Any damage to the Property beyond normal wear and tear
- Unpaid rent or utilities
- Breach of any terms of this Agreement

### Refund of Security Deposit

Upon termination of this Agreement and vacation of the Property, the Landlord shall refund the Security Deposit to the Tenant within 30 days, after deducting any amounts due for:
- Rent arrears
- Utility bills
- Damages to the Property (beyond normal wear and tear)

The Security Deposit shall not be adjusted against the last month's rent without mutual written consent.

---

## 5. Permitted Use

The Tenant shall use the Property exclusively for the following business purpose:

**Business Purpose:** __BUSINESS_PURPOSE__

The Tenant shall:
- Conduct lawful business activities only
- Obtain all necessary licenses and permits for the business
- Comply with local municipal laws, building bye-laws, and Shop and Establishment Act regulations
- Not use the Property for any illegal, immoral, or hazardous activities
- Not cause nuisance to neighbors or other occupants of the building

---

## 6. Utilities and Services

### Tenant's Responsibility

The Tenant shall be responsible for payment of the following utilities and services:
- Electricity charges (separate meter or as per consumption)
- Water charges (if separately metered or as per building rules)
- Internet and telecommunication services
- Cleaning and housekeeping of the rented premises

### Landlord's Responsibility

The Landlord shall ensure:
- Availability of basic utilities (electricity, water connections)
- Proper functioning of common amenities (if applicable)
- Building maintenance charges (unless otherwise agreed)

---

## 7. Maintenance and Repairs

### Tenant's Responsibilities

The Tenant shall:
- Keep the Property clean and in good condition
- Perform routine maintenance and minor repairs
- Maintain interior fixtures, fittings, and furnishings
- Report any major defects or damages to the Landlord promptly
- Hand over the Property in the same condition as received (subject to normal wear and tear)

### Landlord's Responsibilities

The Landlord shall:
- Maintain the structural integrity of the Property (walls, roof, foundation)
- Repair major defects in plumbing, electrical wiring, and sanitary installations
- Ensure common areas (stairs, lifts, corridors) are maintained (if applicable)
- Address issues that affect habitability of the Property

### Alterations

The Tenant shall not make any structural alterations, additions, or modifications to the Property without prior written consent of the Landlord. Minor non-structural changes (such as painting, installing removable fixtures) may be permitted with Landlord's consent.

---

## 8. GST and Tax Obligations

### GST (Goods and Services Tax)

> **Note:** GST at 18% is applicable on commercial rent as per GST Act. However, for small rental agreements with individual landlords (not registered businesses), GST may not be charged if the Landlord is not GST-registered.

- If the Landlord is GST-registered, the rent quoted is **exclusive/inclusive** *(specify)* of GST, and the Landlord shall issue GST-compliant invoices.
- If the Landlord is not GST-registered, no GST shall be charged.

### TDS (Tax Deducted at Source)

> **Note:** As per Section 194-IB of the Income Tax Act, if the annual rent exceeds ₹2,40,000 (₹20,000/month), the Tenant is required to deduct TDS at 10% and deposit it with the Income Tax Department.

- At the rent of ₹__MONTHLY_RENT__/month, if the annual rent equals or exceeds ₹2,40,000, the Tenant may need to deduct TDS.
- If TDS is applicable, the Tenant shall provide TDS certificates (Form 16C) to the Landlord.
- The Landlord's PAN is not mandatorily required for this agreement, but may be needed for TDS compliance if applicable.

---

## 9. Termination and Notice Period

### Termination by Either Party

Either party may terminate this Agreement by providing written notice of **__NOTICE_PERIOD__ days** to the other party.

### Early Termination

- This Agreement may be terminated before the End Date by mutual written consent of both parties.
- In case of breach of any material terms of this Agreement by either party, the non-breaching party may terminate the Agreement with immediate effect after providing written notice.

### Vacation of Property

Upon termination, the Tenant shall:
- Vacate the Property on or before the termination date
- Remove all belongings and restore the Property to its original condition (normal wear and tear excepted)
- Clear all pending rent, utility bills, and other dues
- Hand over all keys and access devices to the Landlord

---

## 10. Force Majeure

In the event of force majeure circumstances (including but not limited to natural disasters, pandemics, government lockdowns, civil unrest, war, or any other event beyond the reasonable control of either party), both parties shall discuss in good faith:
- Temporary suspension or reduction of rent
- Extension of the Agreement period
- Early termination without penalties

Neither party shall be held liable for failure to perform obligations under this Agreement due to force majeure events.

---

## 11. Dispute Resolution

### Amicable Settlement

In case of any dispute, claim, or disagreement arising out of or relating to this Agreement, both parties shall first attempt to resolve the matter amicably through mutual discussion and negotiation.

### Legal Jurisdiction

If the dispute cannot be resolved amicably, it shall be subject to the exclusive jurisdiction of the courts in **__CITY__**, India.

---

## 12. Stamp Duty and Registration

### Stamp Duty

This Agreement shall be executed on appropriate stamp paper as per the stamp duty rates applicable in the state where the Property is located. The cost of stamp duty shall be borne by **both parties equally** *(or as mutually agreed)*.

> **Note:** Stamp duty rates vary by state (typically 0.25% to 4% of total rent for 11-month agreements). Please verify the applicable rate for your state.

### Registration

As this is an 11-month agreement, registration with the Sub-Registrar is generally **not mandatory** in most states. However, parties may choose to register this Agreement voluntarily for additional legal validity.

---

## 13. Miscellaneous Provisions

### Entire Agreement

This Agreement constitutes the entire understanding between the parties and supersedes all prior negotiations, representations, or agreements, whether written or oral.

### Amendments

No amendment, modification, or waiver of any provision of this Agreement shall be valid unless in writing and signed by both parties.

### Severability

If any provision of this Agreement is held to be invalid or unenforceable, the remaining provisions shall continue in full force and effect.

### Notices

All notices under this Agreement shall be in writing and shall be deemed to have been duly given if delivered personally or sent by registered post to the addresses mentioned in this Agreement.

### Binding Effect

This Agreement shall be binding upon and inure to the benefit of the parties and their respective heirs, executors, administrators, legal representatives, successors, and permitted assigns.

---

## 14. Additional Terms

*(Add any additional terms or special conditions agreed upon by both parties)*

---

---

## Execution

**IN WITNESS WHEREOF**, the parties have executed this Agreement on the date first written above.

---

### Landlord's Signature

**Name:** __LANDLORD_NAME__

**Signature:** \_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_

**Date:** \_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_

---

### Tenant's Signature

**Name:** __TENANT_NAME__

**Signature:** \_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_

**Date:** \_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_

---

### Witnesses (Optional but Recommended)

#### Witness 1

**Name:** \_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_

**Address:** \_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_

**Signature:** \_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_

**Date:** \_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_

---

#### Witness 2

**Name:** \_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_

**Address:** \_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_

**Signature:** \_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_

**Date:** \_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_

---

## Important Disclaimers

> **This template is designed for small startups and buddy businesses renting modest office spaces with rent ≤ ₹20,000/month.**

> **This template is provided for informational purposes only and does not constitute legal advice. While it has been designed to be legally adequate for simple commercial rentals, we strongly recommend:**
> - Reviewing this agreement with a qualified lawyer if you have specific concerns
> - Verifying state-specific stamp duty rates and requirements
> - Ensuring compliance with local municipal laws and regulations
> - Understanding your GST and TDS obligations

> **This template is released under CC0-1.0 (Public Domain). You are free to use, modify, and distribute it without attribution.**

---

*Generated using the [Rental Agreement Template](https://github.com/YOUR_USERNAME/rental-agreement-template)*
